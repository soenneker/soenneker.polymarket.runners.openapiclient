...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string sourcesDirectory = Path.Combine(gitDirectory, "openapi.sources");
        await _directoryUtil.Create(sourcesDirectory, cancellationToken: cancellationToken);

        (string Prefix, string ConfigKey, string Url, string Extension)[] sources =
        [
            ("bridge", "Bridge", "https://docs.polymarket.com/api-spec/bridge-openapi.yaml", ".yaml"),
            ("data", "Data", "https://docs.polymarket.com/api-spec/data-openapi.yaml", ".yaml"),
            ("perps", "Perps", "https://docs.polymarket.com/api-spec/perps-openapi.json", ".json"),
            ("clob", "Clob", "https://docs.polymarket.com/api-spec/clob-openapi.yaml", ".yaml"),
            ("combos-rfq", "CombosRfq", "https://docs.polymarket.com/api-spec/combos-rfq-openapi.yaml", ".yaml"),
            ("gamma", "Gamma", "https://docs.polymarket.com/api-spec/gamma-openapi.yaml", ".yaml"),
            ("relayer", "Relayer", "https://docs.polymarket.com/api-spec/relayer-openapi.yaml", ".yaml")
        ];

        var mergeInputs = new List<(string prefix, string filePath)>(sources.Length);

        foreach ((string prefix, string configKey, string defaultUrl, string extension) in sources)
        {
            string sourcePath = Path.Combine(sourcesDirectory, $"{prefix}{extension}");
            await _fileUtil.DeleteIfExists(sourcePath, cancellationToken: cancellationToken);

            string url = _configuration[$"Polymarket:OpenApiUrls:{configKey}"] ?? defaultUrl;
            string? downloadedPath = await _fileDownloadUtil.Download(url, sourcePath, fileExtension: extension, cancellationToken: cancellationToken);
        ...
        await _fileUtil.Write(targetFilePath, _openApiMerger.ToJson(merged), true, cancellationToken);
        ...
        await _kiotaUtil.Generate(fixedFilePath, "PolymarketOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
